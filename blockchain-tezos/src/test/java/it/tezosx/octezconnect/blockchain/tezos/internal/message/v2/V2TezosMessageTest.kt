package it.tezosx.octezconnect.blockchain.tezos.internal.message.v2
...
import it.tezosx.octezconnect.blockchain.tezos.data.operation.TezosEndorsementOperation
import it.tezosx.octezconnect.blockchain.tezos.data.operation.TezosOperation
...
internal class V2TezosMessageTest {
    ...
    fun setup() {
        ...
        every { dependencyRegistry.extend().tezosWallet.address(any()) } answers {
            Result.success(
                firstArg()
            )
        }
    ...
    private fun versionedWithBeacon(
        ...
        listOf(
            ...
            createPermissionResponsePair(
                version = version,
                senderId = senderId,
                destination = destination
            ),
            createOperationResponsePair(
                version = version,
                senderId = senderId,
                destination = destination
            ),
            createSignPayloadResponsePair(
                version = version,
                senderId = senderId,
                destination = destination
            ),
            createBroadcastResponsePair(
                version = version,
                senderId = senderId,
                destination = destination
            ),
    ...
    private fun createPermissionResponseJsonPair(
        ...
        network: TezosNetwork = TezosNetwork.Custom(),
        appMetadata: V2TezosAppMetadata = V2TezosAppMetadata("id", "name", "icon"),
        scopes: List<TezosPermission.Scope> = emptyList(),
        ...
        PermissionV2TezosResponse(
            ...
            notification
                ...
                PermissionTezosResponse(
                    ...
                    destination,
                    Tezos.IDENTIFIER,
                    TezosAccount(publicKey, network, publicKey, publicKey),
                    scopes,
                    appMetadata.toAppMetadata(),
                    threshold,
                    notification
