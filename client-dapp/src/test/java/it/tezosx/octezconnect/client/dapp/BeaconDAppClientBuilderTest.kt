...
import it.tezosx.octezconnect.core.internal.storage.sharedpreferences.SharedPreferencesStorage
...
internal class BeaconDAppClientBuilderTest {
    ...
    fun `builds BeaconWalletClient with default settings`() {
        runBlocking {
            ...
            val beaconWalletClient = BeaconDAppClient.Builder(app.name).apply {
            ...
            coVerify(exactly = 1) {
                beaconSdk.add(
                    ...
                    blockchains,
                    ofType(SharedPreferencesDAppClientStorage::class),
    ...
    fun `builds BeaconWalletClient with default settings when used as builder function`() {
        runBlocking {
            ...
            val beaconWalletClient = BeaconDAppClient(app.name) {
                support(*blockchains.toTypedArray())
            }
            val defaultConnections = emptyList<Connection>()
