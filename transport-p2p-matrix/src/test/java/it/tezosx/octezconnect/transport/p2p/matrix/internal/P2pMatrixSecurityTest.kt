...
import it.tezosx.octezconnect.core.internal.crypto.data.SessionKeyPair
...
internal class P2pMatrixSecurityTest {
    ...
    fun setup() {
        ...
        p2pMatrixSecurity = P2pMatrixSecurity(app, crypto)
    }
    ...
    @Test
    fun `creates user ID`() {
        val userId = p2pMatrixSecurity.userId().getOrThrow()
    ...
    fun `returns failure result if user ID could not be created`() {
        ...
        val userId = p2pMatrixSecurity.userId()
    ...
    fun `creates password`() {
        val password = p2pMatrixSecurity.password().getOrThrow()
    ...
    fun `returns failure result if password could not be created`() {
        ...
        val password = p2pMatrixSecurity.password()
    ...
    fun `creates device ID`() {
        val deviceId = p2pMatrixSecurity.deviceId().getOrThrow()
    ...
    fun `encrypts pairing payload with given key`() {
        ...
        val encrypted = p2pMatrixSecurity.encryptPairingPayload(publicKey, pairingPayload).getOrThrow()
    ...
    fun `returns failure result if pairing payload could not be encrypted`() {
        ...
        val encrypted = p2pMatrixSecurity.encryptPairingPayload(publicKey, pairingPayload)
    ...
    fun `encrypts message with shared key`() {
        ...
        val encrypted = p2pMatrixSecurity.encrypt(publicKey, message).getOrThrow()
    ...
    fun `returns failure result if message could not be encrypted with shared key`() {
        ...
        val encrypted = p2pMatrixSecurity.encrypt(publicKey, message)
    ...
    fun `decrypts message with shared key`() {
        ...
        val decrypted = p2pMatrixSecurity.decrypt(publicKey, message).getOrThrow()
    ...
    fun `returns failure result if message could not be decrypted with shared key`() {
        ...
        val decrypted = p2pMatrixSecurity.decrypt(publicKey, message)
    ...
    fun `creates client session key pair once`() {
        ...
        p2pMatrixSecurity.encrypt(publicKey, message).getOrThrow()
        p2pMatrixSecurity.encrypt(publicKey, message).getOrThrow()
    ...
    fun `creates server session key pair once`() {
        ...
        p2pMatrixSecurity.decrypt(publicKey, message).getOrThrow()
        p2pMatrixSecurity.decrypt(publicKey, message).getOrThrow()
